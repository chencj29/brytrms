...
@Component
@Lazy(false)
public class CarouselTask implements DistributionTaskInterface {
    ...
    public void execute() {
        try {
            Map<String, Object> conditions = ResourceDistributionContainer.getQueue(ResourceType.CAROUSEL).poll();
            if (conditions != null) resourceSharingService.distCarouselAuto(conditions);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
