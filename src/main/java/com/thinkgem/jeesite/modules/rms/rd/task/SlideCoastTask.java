...
@Component
@Lazy(false)
public class SlideCoastTask implements DistributionTaskInterface {
    ...
    public void execute() {
        try {
            Map<String, Object> conditions = ResourceDistributionContainer.getQueue(ResourceType.SLIDE_COAST).poll();
            if (conditions != null) resourceSharingService.distSlideCoastAuto(conditions);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
