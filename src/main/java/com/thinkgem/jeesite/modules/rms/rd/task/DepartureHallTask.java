...
@Component
@Lazy(false)
public class DepartureHallTask implements DistributionTaskInterface {
    ...
    public void execute() {
        try {
            Map<String, Object> conditions = ResourceDistributionContainer.getQueue(ResourceType.DEPARTURE_HALL).poll();
            if (conditions != null) resourceSharingService.distDepartureHallAuto(conditions);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
