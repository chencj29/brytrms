...
@Component
@Lazy(false)
public class BoardingGateTask implements DistributionTaskInterface {
    ...
    public void execute() {
        try {
            Map<String, Object> conditions = ResourceDistributionContainer.getQueue(ResourceType.BOARDING_GATE).poll();
            if (conditions != null) resourceSharingService.distBoardingGateAuto(conditions);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
