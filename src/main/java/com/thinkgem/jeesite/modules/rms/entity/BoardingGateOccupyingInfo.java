 ...
 */
@Oci
@Monitor(desc = "登机口占用信息", tableName = "rms_boarding_gate_oci", service = BoardingGateOccupyingInfoService.class, socketNS = "/global/dynamics")
public class BoardingGateOccupyingInfo extends DataEntity<BoardingGateOccupyingInfo> {
    ...
    @Label("预计占用登机口个数")
    @MonitorField(desc = "预计占用登机口个数")
    private Integer expectedBoardingGateNum;        // 预计占用登机口个数
    @Label("实际占用登机口个数")
    @MonitorField(desc = "实际占用登机口个数")
    private Integer actualBoardingGateNum;        // 实际占用登机口个数
    @Label("登机口编号(国内)")
    @MonitorField(desc = "登机口编号(国内)")
    private String inteBoardingGateCode;        // 登机口编号(国内)
    @Label("登机口编号(国际)")
    @MonitorField(desc = "登机口编号(国际)")
    private String intlBoardingGateCode;        // 登机口编号(国际)
    @Label("预计开始占用时间")
    ...
    public BoardingGateOccupyingInfo() {
        super();
    }

    public BoardingGateOccupyingInfo(String id) {
        super(id);
    }
    ...
    public Integer getExpectedBoardingGateNum() {
        return expectedBoardingGateNum;
    }

    public void setExpectedBoardingGateNum(Integer expectedBoardingGateNum) {
        this.expectedBoardingGateNum = expectedBoardingGateNum;
    }

    public Integer getActualBoardingGateNum() {
        return actualBoardingGateNum;
    }

    public void setActualBoardingGateNum(Integer actualBoardingGateNum) {
        this.actualBoardingGateNum = actualBoardingGateNum;
    }

    @Length(min = 0, max = 100, message = "登机口编号(国内)长度必须介于 0 和 100 之间")
    ...
    @Length(min = 0, max = 100, message = "登机口编号(国际)长度必须介于 0 和 100 之间")
    ...
    public String toString() {
        ...
        if (expectedBoardingGateNum != null) msg.append("预计占用登机口个数:").append(expectedBoardingGateNum).append(",");
        if (actualBoardingGateNum != null) msg.append("实际占用登机口个数:").append(actualBoardingGateNum).append(",");
        if (inteBoardingGateCode != null) msg.append("登机口编号(国内):").append(inteBoardingGateCode).append(",");
        if (intlBoardingGateCode != null) msg.append("登机口编号(国际):").append(intlBoardingGateCode).append(",");
        if (expectedStartTime != null) msg.append("预计开始占用时间:").append(DateUtils.formatDate(expectedStartTime, "yyyy-MM-dd HH:mm")).append(",");
        if (expectedOverTime != null) msg.append("预计结束占用时间:").append(DateUtils.formatDate(expectedOverTime, "yyyy-MM-dd HH:mm"));
