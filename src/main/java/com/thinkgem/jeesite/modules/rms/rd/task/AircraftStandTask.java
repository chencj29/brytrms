...
@Component
@Lazy(false)
public class AircraftStandTask implements DistributionTaskInterface {
    ...
    public void execute() {
        try {
            Map<String, Object> conditions = ResourceDistributionContainer.getQueue(ResourceType.AIRCRAFT_STND).poll();
            if (conditions != null) resourceSharingService.distGateAuto(conditions);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
