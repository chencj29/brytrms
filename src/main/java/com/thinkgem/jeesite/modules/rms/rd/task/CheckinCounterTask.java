...
@Component
@Lazy(false)
public class CheckinCounterTask implements DistributionTaskInterface {
    ...
    public void execute() {
        try {
            Map<String, Object> conditions = ResourceDistributionContainer.getQueue(ResourceType.CHECKIN_COUNTER).poll();
            if (conditions != null) resourceSharingService.distCheckingCounterAuto(conditions);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
