...
//		System.out.println("dataScopeFilter: " + sqlString.toString());
     ...
     * @return 标准连接条件对象
     */
    public static String dataScopeFilterNew(User user, boolean... isPair) {
        ...
        if (!user.isAdmin()) {
            // 拼接航空公司ID 拼接服务提供者ID
            String companyIds = "", serviceIds = "";
            //处理user中不能常获取role中的数据时
            if (Collections3.isEmpty(user.getRoleList())){
                user = UserUtils.get(user.getId());
            }

            for (Role role : user.getRoleList()) {
                if (companyIds.length() > 0) companyIds += ",";
                if (role.getCompanyIdList() != null && role.getCompanyIdList().size() > 0) {
                    companyIds += role.getCompanyIdList().stream().map(s -> "'" + s + "'").reduce((s1, s2) -> s1.concat(",").concat(s2)).get();
                }
                //
                if (serviceIds.length() > 0) serviceIds += ",";
                if (role.getServiceIdList() != null && role.getServiceIdList().size() > 0) {
                    serviceIds += role.getServiceIdList().stream().map(s -> "'" + s + "'").reduce((s1, s2) -> s1.concat(",").concat(s2)).get();
                }
            }
            companyIds = StringUtils.isNotBlank(companyIds) ? companyIds : "'Null'";
            serviceIds = StringUtils.isNotBlank(serviceIds) ? serviceIds : "'Null'";

            if (StringUtils.endsWith(companyIds, ",")) companyIds = StringUtils.removeEnd(companyIds, ",");
            if (StringUtils.endsWith(serviceIds, ",")) serviceIds = StringUtils.removeEnd(serviceIds, ",");

            sqlString.append(" OR a.flight_company_code IN (").append(companyIds).append(") ");
            if (isPair != null && isPair.length > 0 && isPair[0])
                sqlString.append(" OR a.flight_company_code2 IN (").append(companyIds).append(") ");
            sqlString.append(" OR a.agent_code IN (").append(serviceIds).append(") ");
        }
        if (StringUtils.isNotBlank(sqlString.toString())) {
            return " AND (" + sqlString.substring(4) + ")";
        }
    ...
    /**
     * 数据范围过滤 只用于运输服务/运输行李中航班过滤（权限）
     *
     * @param user 当前用户对象，通过“entity.getCurrentUser()”获取
     * @return 标准连接条件对象
     */
    public static String dataScopeFilterSpecial(User user, String... isVip) {
        StringBuilder sqlString = new StringBuilder();
        ...
        if (!user.isAdmin()) {
            ...
            for (Role role : user.getRoleList()) {
                if (companyIds.length() > 0) companyIds += ",";
                if (role.getCompanyIdList() != null && role.getCompanyIdList().size() > 0) {
                    companyIds += role.getCompanyIdList().stream().map(s -> "'" + s + "'").reduce((s1, s2) -> {
            ...
            if (isVip != null && isVip.length > 0) {
                sqlString.append(" a.aircorp IN (").append(companyIds).append(") ");
            } else {
                sqlString.append(" a.flight_company_code IN (").append(companyIds).append(") ");
            }
        }
        if (StringUtils.isNotBlank(sqlString.toString())) {
