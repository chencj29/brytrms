...
var windowapi = undefined;
try{
    windowapi = frameElement.api;
}catch(e){}

var W = undefined;//内容页中调用窗口实例对象接口
try{
    W = windowapi.opener;
}catch(e){}
...
function workflowDetailWindow(obj) {
	var width = obj.width ? obj.width : 700;
	...
	$.dialog({
		...
		lock: true,
		width: width,
		height: height, lock : true, background: '#000', opacity: 0.5,
        resize: true, esc: false, max:true, min:true,
		title: obj.title,
		opacity: 0.3,
		cache: false,
//		button : [{
 ...
 * @param saveurl
 */
function openuploadwin(title, url,name,width, height, _zIndex) {
	gridname=name;
	$.dialog({
	    content: 'url:'+url,
	    cache:false,
	    esc:false,
	    zIndex: _zIndex || 14444,
	    button: [
	        {
	            name: "开始上传",
	            callback: function(){
	...
	if (typeof (width) == 'undefined'&&typeof (height) != 'undefined')
	{
		if(typeof(windowapi) == 'undefined'){
			$.dialog({
				...
				cache:false,
				lock : true,
				background: "#eee", opacity: 0.5, max:true, min:true, drag:true, resize:true, parent: windowapi,
				width: 'auto',
			    height: height
			}).zindex();
		}else{
			$.dialog({
				...
				esc:false,
				cache:false,
				background: "#eee", opacity: 0.5, max:true, min:true, drag:true, resize:true,
				parent:windowapi,
				lock : true,
				width: 'auto',
			    height: height
	...
	if (typeof (height) == 'undefined'&&typeof (width) != 'undefined')
	{
		if(typeof(windowapi) == 'undefined'){
			$.dialog({
				...
				lock : true,
				esc:false,
				width: width,background: "#eee", opacity: 0.5, max:true, min:true, drag:true, resize:true, parent:windowapi,
				cache:false,
			    height: 'auto'
			}).zindex();
		}else{
	...
	if (typeof (width) == 'undefined'&&typeof (height) == 'undefined')
	{
		if(typeof(windowapi) == 'undefined'){
			$.dialog({
				...
				lock : true,
				width: 'auto',
				esc:false,background: "#eee", opacity: 0.5, max:true, min:true, drag:true, resize:true, parent:windowapi,
				cache:false,
			    height: 'auto'
			}).zindex();
		}else{
			$.dialog({
				...
				title : title,
				esc: false,
				lock : true,background: "#eee", opacity: 0.5, max:true, min:true, drag:true, resize:true,
				parent:windowapi,
				width: 'auto',
				cache:false,
			    height: 'auto'
	...
	if (typeof (width) != 'undefined'&&typeof (height) != 'undefined')
	{
		if(typeof(windowapi) == 'undefined'){
			$.dialog({
			    ...
			    esc:false,
				content: 'url:'+url,
				title : title,background: "#eee", opacity: 0.5, max:true, min:true, drag:true, resize:true,parent:windowapi,
				cache:false,
				lock : true
			}).zindex();
		}else{
			$.dialog({
				...
				content: 'url:'+url,
				parent:windowapi,
				title : title,background: "#eee", opacity: 0.5, max:true, min:true, drag:true, resize:true,
				cache:false,
				lock : true
			}).zindex();
		}
...
function saveObj() {
	try {
		iframe.ckeditor_detail.updateElement();
	} catch(e) {}
...
function opensubwin(title, url, saveurl, okbutton, closebutton, _width, _height, _zIndex, _id) {
	$.dialog({
		content: 'url:' + url, title: title, lock: true,
		background: '#000', opacity: 0.5, resize: true,
		esc: false, max: true, min: true,
		width: _width || 900, height: _height || 500,
		padding: 0, modal: true, drag: false,
		zIndex: _zIndex || 12000, id: _id || "GENERALSUBWIN",
		button : [{
			name: okbutton,
			callback: function() {
				iframe = this.iframe.contentWindow;
