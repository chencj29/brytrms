(function ($) {
    ...
    var extendedOptions = {
        ...
        filterMatcher: function (data) {
            ...
            var opts = state.options;
            data.unMatchedRows = []; //  store unMatch rows - added by xiaowu 2016-08-23
            if (opts.filterRules.length) {
                ...
                if (name == 'treegrid') {
                    ...
                    for (var i = 0; i < data.rows.length; i++) {
                        ...
                        if (isMatch(row, i)) {
                            rows.push(row);
                        } else data.unMatchedRows.push(row);
                    }
                }
                data = { total: data.total - (data.rows.length - rows.length), rows: rows, unMatchedRows: data.unMatchedRows };
            }
            return data;
            ...
            function isMatch (row, index) {
                ...
                for (var i = 0; i < rules.length; i++) {
                    ...
                    if (col && col.formatter) {
                        source = col.formatter(row[rule.field], row, index, col.field);
                    }
                    if (source == undefined) {
                        source = '';
                    }
