 ...
 * blinkFlagOp:已过当前时间的不闪烁，默认为false
 *
 * 插件依赖:
 *  1) jQuery
 *  2) jQuery UI
...
((function ($) {
    ...
    $.fn.gantt.defaults = {
        ...
        collisionDetectionHandler: function (event, ui) {
            var _this = this, blinkFlagOp= $(".gantt").data('options').blinkFlagOp;
            if (overlaps) {
                ...
                $(".airplane-wrapper > div").filter(function () {
                    ...
                    //过滤已完成的 不闪烁
                    var blinkFlag = false;
                    if (blinkFlagOp && $(this).data("o")) {
                        var aircraftOverDate = $(this).data("o").aircraftOverDate;
                        if(aircraftOverDate && moment(aircraftOverDate).diff() < 0) blinkFlag = true;
                    }

                    if (overlaps(ui.helper, $(this), (_this.collisionThresholds / _this.unitFactor) * _this.unitWidth) && !blinkFlag) {
