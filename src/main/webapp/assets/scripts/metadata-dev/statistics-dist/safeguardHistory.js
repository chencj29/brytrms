...
$(function () {
    //给时间控件加上样式
    $("#safeguardHistoryListtb #planDate_begin").attr("class", "Wdate").attr("style", "height:20px;width:90px;").click(function () {
        WdatePicker({dateFmt: 'yyyy-MM-dd', maxDate: '#F{$dp.$D(\'planDate_end\')}'});
    });
    $("#safeguardHistoryListtb #planDate_end").attr("class", "Wdate").attr("style", "height:20px;width:90px;").click(function () {
...
function reloadsafeguardHistoryList() {
    $('#datagrid').datagrid('reload');
}

function safeguardHistoryListsearch() {
    ...
    //$("#sourceType").combobox()
    $('#safeguardHistoryListtb').find('*').each(function () {
...
function downloadExcel() {
    ...
    location.href = ctx + '/rms/statistics/safeguardHistoryExpExcel?' + parseParam(queryParams);
}
