const TPL_NAME = 'boarding-gate', moduleContext = "boardingGate", moduleName = '登机口', inoutTypeCode = 'C';
var xAxis = enhanceXAxis(), yAxis = $utils.ajaxLoadFragments(ctx + '/rms/boardingGate/listJson'), checkingIndex;
...
var gateGanttOptions = {
        ...
        ajaxUrl: ctx + '/rms/rs/disted_boarding_gate_Json',
        ...
        move2HoldAreaCallback: function (oData) {
            if (oData.flightDynamicId) { // 用户手动拖动到「预留区」
                ...
                tipMessage = `您确定要清除「${selectedRow.flightCompanyCode + selectedRow.flightDynamicCode}」航班的「${originAxis.text}」号登机口吗?`;
                ...
                layer.confirm(tipMessage, function (index) {
                    ...
                    $.post(ctx + '/rms/rs/clearBoardingGateCodeByNature', $.extend({}, params, extraData), function (data) {
        ...
        dblClick: function (event) {
            commonSetActualTimeHandler(TPL_NAME, getFormatTitle(enhanceGetDGSelectedRow(event.data.flightDynamicId, "flightDynamicId")), '登机口 - 实际占用时间设置', '/rms/rs/setBoardingGateActualTime')
        },
        leftAxisClickHandler: function () {
            ...
            if ($(this).attr("_available") === '0') {
                layer.msg('当前登机口「' + $(this).text() + '」不可用，无法分配！');
                return false;
            }
            dist_boarding_gate_manually($(this).text());
        }
...
$(function () {
    ...
    if (flagAutoAllocation)
        toolbars.push({
            ...
            handler: () => commonDistributedResourceAuto('/rms/rs/boarding_gate_dist_auto', moduleName)
        });
    ...
    if (flagManualAllocation) {
        toolbars.push({
            text: '手动分配', iconCls: 'icon-bullet_wrench', handler: function () {
                ...
                //     if (selectedRow) {
                //         if (multiModeGanttDistributedAvailableChecking(moduleName, '/rms/rs/getBoardingGateOccupyingTime?flightDynamicId=' + selectedRow.flightDynamicId, selectedRow)) {
                //             if (selectedRow.flightDynamicNature !== '3') {
                //
                //                 var availableInte = $utils.ajaxLoadFragments(ctx + '/rms/boardingGate/list_json_by_condition?nature=1'),
                //                     availableIntl = $utils.ajaxLoadFragments(ctx + '/rms/boardingGate/list_json_by_condition?nature=2'),
                ...
                //                         $("#cbInteCarouselCode").select2($.extend({}, {placeholder: '请选择国内航段登机口'}, select2Opts));
                //                         $("#cbIntlCarouselCode").select2($.extend({}, {placeholder: '请选择国际航段登机口'}, select2Opts));
                //
                ...
                //                             force: true
                //                         };
                //                         commonManuallyDistributionFn(ctx + '/rms/rs/boarding_gate_manual_dist', ctx + '/rms/rs/boarding_gate_manual_dist', pArgs, gArgs, checkingIndex)
                //                         return true;
                //                     }
                //                 });
                //             } else dist_boarding_gate_manually();
                //         }
    ...
    if (flagEndFlightOccupation) {
        toolbars.push({
            text: '实际占用时间录入', iconCls: 'icon-stop', handler: function () {
                var selectedRow = datagrid.datagrid('getSelected');
                if (selectedRow) commonSetActualTimeHandler(TPL_NAME, getFormatTitle(selectedRow), '登机口 - 实际占用时间设置', '/rms/rs/setBoardingGateActualTime')
    ...
    datagrid = $("#datagrid").datagrid({
        rownumbers: true,
        url: ctx + "/rms/rs/boarding-gate-list",
...
function selectFn(rowIndex, selectedRow) {
    if (multiModeGanttDistributedAvailableChecking(moduleName, '/rms/rs/getBoardingGateOccupyingTime?flightDynamicId=' + selectedRow.flightDynamicId, selectedRow)) {
        if (selectedRow.flightDynamicNature !== '3') {
            ...
            var settings = $(".gantt").data("options"), oldCarouselCode;
            if (!selectedRow) return false;

            if (!multiModeGanttDistributedAvailableChecking(moduleName, '/rms/rs/getBoardingGateOccupyingTime?flightDynamicId=' + selectedRow.flightDynamicId, selectedRow)) return;
            ...
            if (flagManualAllocation) {
                $("#carouseSave").click(function () {
                    ...
                    var pArgs = {
                        flightDynamicId: selectedRow.flightDynamicId,
                        inte: cintec, intl: cintlc, nature: selectedRow.flightDynamicNature, force: false
                    };
                    var gArgs = {
                        selectedRowId: selectedRow.id,
                        nInte: cintec, nIntl: cintlc, oInte: oldCarouselCode.inte,
                        oIntl: oldCarouselCode.intl, nature: selectedRow.flightDynamicNature, force: true
                    };
                    commonManuallyDistributionFn(ctx + '/rms/rs/boarding_gate_manual_dist', ctx + '/rms/rs/boarding_gate_manual_dist', pArgs, gArgs, checkingIndex, null)
                })
            }
...
function clearCallbackFn() {
    ...
    if (selectedRow) {
        if (selectedRow.inteCarouselCode || selectedRow.intlCarouselCode) {
            layer.confirm(`是否要清除当前航班「${selectedRow.flightCompanyCode + selectedRow.flightDynamicCode}」分配的登机口？`, {
                icon: 3,
                title: '提示'
            }, function (index) {
                $.post(ctx + '/rms/rs/clearBoardingGateCode', $.extend({}, {flightDynamicId: selectedRow.flightDynamicId}, extraData), function (data) {
                    if (data.code == 1) {
                        layer.msg(data.message);
...
var _socket_uuid = $uuid(), extraData = {random: _socket_uuid},
    ...
    entityMapping = {inteBoardingGateCode: "inteCarouselCode", intlBoardingGateCode: "intlCarouselCode"};
...
socket.on("modification", function (data) {
    ...
    _.defer(function (data) {
        let _entity = $.parseJSON(data.data), _changeList = $.parseJSON(data.changeList),
            _identify = data.identify, _compareId = _entity.id,
        ...
        if (!_.contains(currentUserPrivileges, ALL_PRIVILEGE) && !_.contains(currentUserPrivileges, _entity.flightCompanyCode) &&
            ...
            _identify !== 'BoardingGateOccupyingInfo') return;

        if (_identify === 'BoardingGateOccupyingInfo') _compareId = _entity.flightDynamicId;
        //else if (_identify === 'FlightPlanPair') _compareId = _entity.flightDynimicId2;//用来更新机位的
        ...
        if (_identify === "FlightDynamic" && monitorType === "INSERT") {
            if (_entity.inoutTypeCode === inoutTypeCode) {
                _entity.flightPlan = "";
                $.post(ctx + "/rms/rs/getBoardingGateOccupyingTimeByDynamic", _entity).then(function (data) {
                    var oci = {
                        ...
                        flightDynamicCode: _entity.flightNum, expectedBoardingGateNum: 1
                    };
            ...
            if(_identify === "BoardingGateOccupyingInfo" && data.uuid == _socket_uuid) return;
            ...
            if ((!filtered && rowIndex !== undefined && rowIndex != -1) || (filtered && filteredRowIndex != -1)) {
                ...
                if (_.intersection(_.keys(changeWrapper), [" placeNum"]).length > 0) console.log(changeWrapper);
            }
        }
    }, data);
    ...
    if (isExpand) {
        _.defer(function (data) {
            ...
            if (dynamic.inoutTypeCode === inoutTypeCode) {
                dynamic.flightPlan = '';
                $.ajax({
                    url: ctx + '/rms/boardingGateOccupyingInfo/fetchByFlightDynamicId',
                    ...
                    dynamic = $.extend({}, dynamic, {
                        ...
                        expectedCarouselNum: oci.expectedBoardingGateNum,
                        inteCarouselCode: oci.inteBoardingGateCode,
                        intlCarouselCode: oci.intlBoardingGateCode
                    });
                });
                //更新甘特图
...
function fnAfterInited(resp) {
    ...
    _.each(dynamics, (dynamic) => {
        ...
        wrappers.push(_.extend({}, dynamic, {
            ...
            flightDynamicCode: dynamic.flightNum, expectedCarouselNum: oci.expectedBoardingGateNum,
            inteCarouselCode: oci.inteBoardingGateCode, intlCarouselCode: oci.intlBoardingGateCode
        }));
    });
...
function ganttReload() {
    $(".gantt").gantt('reload', ctx + '/rms/rs/disted_boarding_gate_Json', {
        startDate: _.first(xAxis),
        overDate: _.last(xAxis)
    })
}

function dist_boarding_gate_manually(_aircraft_num) {
    ...
    if (!_aircraft_num) {
        ...
        var confirmLayerIndex = layer.open({
            title: '登机口手动分配',
            btn: ['确定', '取消'],
            area: ['350px', selectedRow.flightDynamicNature === '3' ? '200px' : '180px'],
            type: 1,
            scrollbar: false,
            move: false,
            content: fetchResourceDistTpl(TPL_NAME, {
            ...
            yes: function (index, layero) {
                ...
                commonManuallyDistributionFn(ctx + '/rms/rs/boarding_gate_manual_dist', ctx + '/rms/rs/boarding_gate_manual_dist', pArgs, gArgs, checkingIndex, confirmLayerIndex)
                return true;
            }
        });
    } else {
        ...
        if (draggable_holder) {  // 用户拖动Gantt上的元素
            ...
            if (!newAxisExists) {  // 移动到inte及intl都不存在的数值上
                ...
                let layerPromptMsgTpl = `您确定将「${newAxis.text}」号登机口分配给「${flightNum}」航班吗?`;
                ...
                layer.confirm(layerPromptMsgTpl, function (index) {
                    ...
                    commonManuallyDistributionFn(ctx + '/rms/rs/boarding_gate_manual_dist', ctx + '/rms/rs/boarding_gate_manual_dist', params, gArgs, checkingIndex);
            ...
            if (intes === _aircraft_num || intls === _aircraft_num) { // 清除操作
                tipMessage = `您确定要清除「${flightNum}」航班的「${_aircraft_num}」号登机口吗?`;

                layer.confirm(tipMessage, function (index) {
                    checkingIndex = distributedWaitingDialog();
                    $.post(ctx + '/rms/rs/clearBoardingGateCodeByNature', $.extend({}, {
                        flightDynamicId: selectedRow.flightDynamicId,
                        type: nature
                ...
                tipMessage = `您确定要将编号为「${_aircraft_num}」登机口分配给航班「${flightNum}」吗?`;
                ...
                layer.confirm(tipMessage, function (index) {
                    var oldCarouselCode = {inte: intes, intl: intls}, expectBoardingGateCode = _aircraft_num,
