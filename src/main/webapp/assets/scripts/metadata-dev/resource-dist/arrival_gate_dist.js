const TPL_NAME = 'arrival-gate', moduleContext = 'arrivalGate', moduleName = '到港门', inoutTypeCode = 'J';
var xAxis = enhanceXAxis(), yAxis = $utils.ajaxLoadFragments(ctx + '/rms/arrivalGate/listJson');
...
var gateGanttOptions = {
        ...
        timeline: true,
        ajaxUrl: ctx + '/rms/rs/disted_arrival_gate_Json',
        ...
        move2HoldAreaCallback: function (oData) {
            if (oData.flightDynamicId) { // 用户手动拖动到「预留区」
                ...
                tipMessage = `您确定要清除「${selectedRow.flightCompanyCode + selectedRow.flightDynamicCode}」航班的「${originAxis.text}」号到港门吗?`;
                ...
                layer.confirm(tipMessage, function (index) {
                    ...
                    $.post(ctx + '/rms/rs/clearArrivalGateCodeByNature', $.extend({}, params, extraData), function (data) {
        ...
        dblClick: function (event) {
            commonSetActualTimeHandler(TPL_NAME, getFormatTitle(enhanceGetDGSelectedRow(event.data.flightDynamicId, "flightDynamicId")), '到港门 - 实际占用时间设置', '/rms/rs/setArrivalGateActualTime');
        },
        leftAxisClickHandler: function () {
            ...
            if ($(this).attr("_available") === '0') {
                layer.msg('当前到港门「' + $(this).text() + '」不可用，无法分配！');
                return false;
            }
            dist_arrival_gate_manually($(this).text());
        }
...
$(function () {
        ...
        if (flagAutoAllocation)
            toolbars.push({
                ...
                handler: () => commonDistributedResourceAuto('/rms/rs/arrival_gate_dist_auto', moduleName)
            });
        ...
        if (flagManualAllocation) {
            toolbars.push({
                text: '手动分配', iconCls: 'icon-bullet_wrench', handler: function () {
                    ...
                    if (selectedRow) {
                        if (multiModeGanttDistributedAvailableChecking(moduleName, '/rms/rs/getArrivalGateOccupyingTime?flightDynamicId=' + selectedRow.flightDynamicId, selectedRow)) {
                            if (selectedRow.flightDynamicNature !== '3') {

                                var availableInte = $utils.ajaxLoadFragments(ctx + '/rms/arrivalGate/list_json_by_condition?nature=1'),
                                    availableIntl = $utils.ajaxLoadFragments(ctx + '/rms/arrivalGate/list_json_by_condition?nature=2'),
                                ...
                                layer.open({
                                    ...
                                    content: fetchResourceDistTpl(TPL_NAME, {
                                        ...
                                        availableInte,
                                        availableIntl,
                                        fightDynamicId: selectedRow.flightDynamicId,
                                        nature: selectedRow.flightDynamicNature
                                    }),
                                    success: function (layero, index) {
                                        var select2Opts = {allowClear: true};
                                        $("#cbInteCarouselCode").select2($.extend({}, {placeholder: '请选择国内航段到港门'}, select2Opts));
                                        $("#cbIntlCarouselCode").select2($.extend({}, {placeholder: '请选择国际航段到港门'}, select2Opts));
                                    ...
                                    yes: function (index, layero) {
                                        ...
                                        commonManuallyDistributionFn(ctx + '/rms/rs/arrival_gate_manual_dist', ctx + '/rms/rs/arrival_gate_manual_dist', pArgs, gArgs, checkingIndex)
                                        return true;
                                    }
                                });
                            } else dist_arrival_gate_manually();
                        }
        ...
        if (flagEndFlightOccupation) {
            toolbars.push({
                text: '实际占用时间录入', iconCls: 'icon-stop', handler: function () {
                    var selectedRow = datagrid.datagrid('getSelected');
                    if (selectedRow) {
                        commonSetActualTimeHandler(TPL_NAME, getFormatTitle(selectedRow), '到港门 - 实际占用时间设置', '/rms/rs/setArrivalGateActualTime');
        ...
        datagrid = $("#datagrid").datagrid({
            rownumbers: true,
            url: ctx + "/rms/rs/arrival-gate-list",
...
function clearCallbackFn() {
    ...
    if (selectedRow) {
        if (selectedRow.inteCarouselCode || selectedRow.intlCarouselCode) {
            layer.confirm(`是否要清除当前航班「${selectedRow.flightCompanyCode + selectedRow.flightDynamicCode}」分配的到港门？`, {
                icon: 3,
                title: '提示'
            }, function (index) {
                $.post(ctx + '/rms/rs/clearArrivalGateCode', $.extend({}, {flightDynamicId: selectedRow.flightDynamicId}, extraData), function (data) {
...
var _socket_uuid = $uuid(), extraData = {random: _socket_uuid},
    ...
    entityMapping = {inteArrivalGateCode: "inteCarouselCode", intlArrivalGateCode: "intlCarouselCode"};
...
socket.on("modification", function (data) {
    ...
    _.defer(function (data) {
        ...
        if (!_.contains(currentUserPrivileges, ALL_PRIVILEGE) && !_.contains(currentUserPrivileges, _entity.flightCompanyCode) &&
            ...
            _identify !== 'ArrivalGateOccupyingInfo') return;

        if (_identify === 'ArrivalGateOccupyingInfo') _compareId = _entity.flightDynamicId;
        ...
        if (_identify === "FlightDynamic" && monitorType === "INSERT") {
            if (_entity.inoutTypeCode === inoutTypeCode) {
                ...
                $.post(ctx + "/rms/rs/getCarouselOccupyingTimeByDynamic", _entity).then(function (data) {
                    var oci = {
                        flightDynamicId: _entity.id,
                        flightDynamicNature: data.nature,
                        flightDynamicCode: _entity.flightNum,
                        expectedArrivalGateNum: 1,
                        expectedStartTime: _formatDateTime(data.start),
                        expectedOverTime: _formatDateTime(data.over)
                    };
    ...
    if (isExpand) {
        _.defer(function (data) {
            ...
            if (dynamic.inoutTypeCode === inoutTypeCode) {
                dynamic.flightPlan = '';
                $.ajax({
                    url: ctx + '/rms/arrivalGateOccupyingInfo/fetchByFlightDynamicId',
                    ...
                    dynamic = $.extend({}, dynamic, {
                        ...
                        expectedCarouselNum: oci.expectedArrivalGateNum,
                        inteCarouselCode: oci.inteArrivalGateCode,
                        intlCarouselCode: oci.intlArrivalGateCode
                    });
                });
...
function fnAfterInited(resp) {
    ...
    _.each(dynamics, (dynamic) => {
        ...
        wrappers.push(_.extend({}, dynamic, {
            ...
            flightDynamicCode: dynamic.flightNum, expectedCarouselNum: oci.expectedArrivalGateNum,
            inteCarouselCode: oci.inteArrivalGateCode, intlCarouselCode: oci.intlArrivalGateCode
        }));
    });
...
function ganttReload() {
    $(".gantt").gantt('reload', ctx + '/rms/rs/disted_arrival_gate_Json', {
        startDate: _.first(xAxis),
        overDate: _.last(xAxis)
    })
}

function dist_arrival_gate_manually(_aircraft_num) {
    var selectedRow = datagrid.datagrid('getSelected');
    if (!selectedRow) {
    ...
    if (!multiModeGanttDistributedAvailableChecking(moduleName, '/rms/rs/getArrivalGateOccupyingTime?flightDynamicId=' + selectedRow.flightDynamicId, selectedRow)) return;
    if (!_aircraft_num) {
        ...
        var confirmLayerIndex = layer.open({
            title: '到港门手动分配', btn: ['确定', '取消'], type: 1, scrollbar: false, move: false,
            ...
            yes: function (index, layero) {
                ...
                commonManuallyDistributionFn(ctx + '/rms/rs/arrival_gate_manual_dist', ctx + '/rms/rs/arrival_gate_manual_dist', pArgs, gArgs, checkingIndex, confirmLayerIndex)
                return true;
            }
        });
    } else {
        ...
        if (draggable_holder) {  // 用户拖动Gantt上的元素
            ...
            if (!newAxisExists) {  // 移动到inte及intl都不存在的数值上
                ...
                let layerPromptMsgTpl = `您确定将「${newAxis.text}」号到港门分配给「${flightNum}」航班吗?`;
                ...
                layer.confirm(layerPromptMsgTpl, function (index) {
                    ...
                    commonManuallyDistributionFn(ctx + '/rms/rs/arrival_gate_manual_dist', ctx + '/rms/rs/arrival_gate_manual_dist', params, gArgs, checkingIndex)
                }, () => normalDragStopFn(origin_drag_elem));
            ...
            if (intes === _aircraft_num || intls === _aircraft_num) { // 清除操作
                tipMessage = `您确定要清除「${flightNum}」航班的「${_aircraft_num}」号到港门吗?`;

                layer.confirm(tipMessage, function (index) {
                    checkingIndex = distributedWaitingDialog();

                    $.post(ctx + '/rms/rs/clearArrivalGateCodeByNature', $.extend({}, {
                        flightDynamicId: selectedRow.flightDynamicId,
                        type: nature
                ...
                tipMessage = `您确定要将编号为「${_aircraft_num}」到港门分配给航班「${flightNum}」吗?`;
                ...
                layer.confirm(tipMessage, function (index) {
                    var oldCarouselCode = {inte: intes, intl: intls}, expectArrivalGateCode = _aircraft_num,
