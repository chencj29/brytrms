...
var xAxis = enhanceXAxis(), yAxis = $utils.ajaxLoadFragments(ctx + '/rms/securityCheckin/listJson');
...
var gateGanttOptions = {
        ...
        timeline: true,
        ajaxUrl: ctx + '/rms/rs/disted_security_checkin_Json',
        ...
        move2HoldAreaCallback: function (oData) {
            if (oData.flightDynamicId) { // 用户手动拖动到「预留区」
                ...
                layer.confirm(tipMessage, function (index) {
                    ...
                    $.post(ctx + '/rms/rs/securityCheckin_manual_checking', $.extend({}, selectedRow, extraData), function (data) {
                        ...
                        if (data.code == 1) {
                            ...
                            layer.close(index);
                            updateResourcePosition(selectedRow.id, selectedRow.type = 'inte' ? {inteSecurityCheckinCode: selectedRow.inteSecurityCheckinCode} : {intlSecurityCheckinCode: selectedRow.intlSecurityCheckinCode});
        ...
        dblClick: function (event) {
            commonSetActualTimeHandler(TPL_NAME, getFormatTitle(enhanceGetDGSelectedRow(event.data.flightDynamicId, "flightDynamicId")), '安检口 - 实际占用时间设置', '/rms/rs/setSecurityCheckinActualTime');
        },
...
$(function () {
    ...
    if (flagAutoAllocation)
        toolbars.push({
            ...
            handler: () => commonDistributedResourceAuto('/rms/rs/security_checkin_dist_auto', moduleName)
        });
                                    ...
                                    if (!intes && !intls) { //如果两者皆为空
                                        ...
                                        $.post(ctx + '/rms/rs/securityCheckin_manual_checking', $.extend({}, selectedRow, extraData), function (data) {
                                            ...
                                            if (data.code == 1) {
                                                ...
                                                layer.close(index);
                                                layer.alert(data.message);
                                                updateResourcePosition(selectedRow.id, _.isEmpty(intls) ? {inteSecurityCheckinCode: selectedRow.inteSecurityCheckinCode} : {intlSecurityCheckinCode: selectedRow.intlSecurityCheckinCode});
                                            } else layer.alert(data.message);
                                        });
                                    }
                                    layer.close(index);
    ...
    if (flagEndFlightOccupation) {
        toolbars.push({
            text: '实际占用时间录入', iconCls: 'icon-stop', handler: function () {
                var selectedRow = datagrid.datagrid('getSelected');
                if (selectedRow) commonSetActualTimeHandler(TPL_NAME, getFormatTitle(selectedRow), '安检口 - 实际占用时间设置', '/rms/rs/setSecurityCheckinActualTime')
    ...
    datagrid = $("#datagrid").datagrid({
        ...
        singleSelect: true,
        multiSort: false,
        url: ctx + "/rms/rs/security-checkin-list",
...
function clearCallbackFn() {
    ...
    if (selectedRow) {
        if (selectedRow.inteSecurityCheckinCode || selectedRow.intlSecurityCheckinCode) {
            layer.confirm(`是否要清除「${selectedRow.flightCompanyCode + selectedRow.flightDynamicCode}」航班分配的安检口？`, {
                icon: 3,
                title: '提示'
            }, function (index) {
                $.post(ctx + '/rms/rs/clearSecurityCheckin', $.extend({}, {flightDynamicId: selectedRow.flightDynamicId}, extraData), function (data) {
                    if (data.code == 1) {
                        layer.msg(data.message);
                        ...
                        layer.close(index);
                        updateResourcePosition(selectedRow.id, {
                            inteSecurityCheckinCode: selectedRow.inteSecurityCheckinCode,
                            intlSecurityCheckinCode: selectedRow.intlSecurityCheckinCode
                        })
                    } else layer.alert(data.message);
                });
            });
...
function scroll2CallbackFn() {
    ...
    if (oData) {
        ...
        // if ($(".airplane-wrapper > div").length === 0) ganttReload();
...
function ganttReload() {
    $(".gantt").gantt('reload', ctx + '/rms/rs/disted_security_checkin_Json', {
...
socket.on("modification", function (data) {
    ...
    _.defer(function (data) {
        ...
        if (!_.contains(currentUserPrivileges, ALL_PRIVILEGE) && !_.contains(currentUserPrivileges, _entity.flightCompanyCode) &&
            ...
            _identify !== 'SecurityCheckinOccupyingInfo') return;

        if (_identify === 'SecurityCheckinOccupyingInfo') _compareId = _entity.flightDynamicId;
        ...
        if (_identify === "FlightDynamic" && monitorType === "INSERT") {
            if (_entity.inoutTypeCode === inoutTypeCode) {
                _entity.flightPlan = "";
                $.post(ctx + "/rms/rs/getSecurityCheckinOccupyingTimeByDynamic", _entity).then(function (data) {
                    var oci = {
                        ...
                        flightDynamicCode: _entity.flightNum, expectedSecurityCheckinNum: data.expectedNum
                    };
            ...
            if ((!filtered && rowIndex !== undefined && rowIndex != -1) || (filtered && filteredRowIndex != -1)) {
                ...
                if (isExpand) {
                    ...
                    templateData.expectedNum = templateData.expectedSecurityCheckinNum;
                    var mockGanttData = newTempalteWrapper(templateData, templateData);

                    var inteCodes = $utils.superTrim(templateData.inteSecurityCheckinCode, ""),
                        intlCodes = $utils.superTrim(templateData.intlSecurityCheckinCode, "");
    ...
    if (isExpand) {
        _.defer(function (data) {
            ...
            if (dynamic.inoutTypeCode === inoutTypeCode) {
                dynamic.flightPlan = '';
                $.ajax({
                    url: ctx + '/rms/securityCheckinOccupyingInfo/fetchByFlightDynamicId',
                    ...
                    dynamic = $.extend({}, dynamic, {
                        ...
                        expectedSecurityCheckinNum: oci.expectedSecurityCheckinNum,
                        inteSecurityCheckinCode: oci.inteSecurityCheckinCode,
                        intlSecurityCheckinCode: oci.intlSecurityCheckinCode
                    });
                });
...
function fnAfterInited(resp) {
    ...
    _.each(dynamics, (dynamic) => {
        ...
        wrappers.push(_.extend({}, dynamic, {
            ...
            flightDynamicCode: dynamic.flightNum, expectedSecurityCheckinNum: oci.expectedSecurityCheckinNum,
            inteSecurityCheckinCode: oci.inteSecurityCheckinCode, intlSecurityCheckinCode: oci.intlSecurityCheckinCode
        }))
    });
...
function dist_security_checking_manually(_aircraft_num) {
    ...
    if (!_aircraft_num) {  // 混合航班，弹出界面分配
        ...
        var confirmLayerIndex = layer.open({
            ...
            yes: function (index, layero) {
                oldCarouselCode = {
                    inte: selectedRow.inteSecurityCheckinCode,
                    intl: selectedRow.intlSecurityCheckinCode
                };
                ...
                selectedRow.inteSecurityCheckinCode = cintec ? cintec.join(",") : "";
                selectedRow.intlSecurityCheckinCode = cintlc ? cintlc.join(",") : "";
                ...
                $.post(ctx + '/rms/rs/securityCheckin_manual_checking_4_mixing', $.extend({}, selectedRow, extraData), function (data) {
                    layer.close(checkingIndex);
                    ...
                    if (data.code == 1) {
                        ...
                        layer.alert(data.message);
                        updateResourcePosition(selectedRow.id, {
                            inteSecurityCheckinCode: oldCarouselCode.inte,
                            intlSecurityCheckinCode: oldCarouselCode.intl
                        });
                    } else layer.alert(data.message);
                });
                return true;
        ...
        if (draggable_holder) {  // 用户拖动Gantt上的元素
            ...
            if (!newAxisExists) {  // 移动到inte及intl都不存在的数值上
                ...
                layer.confirm(layerPromptMsgTpl, function (index) {
                    ...
                    var inte_array = _.without(selectedRow.inteSecurityCheckinCode.split(","), ""),
                        intl_array = _.without(selectedRow.intlSecurityCheckinCode.split(","), "");
                    ...
                    //清除首尾多余的逗号
                    selectedRow.inteSecurityCheckinCode = emptyFirstAndLastCommas(selectedRow.inteSecurityCheckinCode);
                    selectedRow.intlSecurityCheckinCode = emptyFirstAndLastCommas(selectedRow.intlSecurityCheckinCode);
                    ...
                    $.post(ctx + '/rms/rs/securityCheckin_manual_checking_4_mixing', $.extend({}, selectedRow, extraData), function (data) {
                        ...
                        if (data.code == 1) {
                            ...
                            layer.alert(data.message);
                            updateResourcePosition(selectedRow.id, {
                                inteSecurityCheckinCode: inte_array.join(","),
                                intlSecurityCheckinCode: intl_array.join(",")
                            });
                        } else layer.alert(data.message);
                    })
                }, () => multiModeDragStopFn(origin_drag_elem));
            ...
            layer.confirm(tipMessage, function (index) {
                ...
                $.post(ctx + '/rms/rs/securityCheckin_manual_checking', $.extend({}, selectedRow, extraData), function (data) {
                    ...
                    if (data.code == 1) {
                        ...
                        layer.alert(data.message);
                        layer.close(index);
                        updateResourcePosition(selectedRow.id, nature === "1" ? {inteSecurityCheckinCode: selectedRow.inteSecurityCheckinCode} : {intlSecurityCheckinCode: selectedRow.intlSecurityCheckinCode});
                    } else layer.alert(data.message);
                });
            }, clearManuallyGanttRefs);
        }
