...
var xAxis = enhanceXAxis4Mock(), yAxis = $utils.ajaxLoadFragments(ctx + '/rms/arrivalGate/listJson');

var gateGanttOptions = {
    auto: true, xAxis: xAxis, yAxis: yAxis, draggableAxis: 'y', collisionThresholds: getThresholdValue(), timeline: true,
    ajaxUrl: ctx + '/rms/rs/disted_arrival_gate_Json_4_mock', ajaxParams: { infoId: $("#infoId").val() }, resizableHandler: false,
    ...
    dblClick: function (event) {
        ...
        }, '到港门 - 实际占用时间设置', '/rms/rs/setArrivalGateActualTime');
    },
...
$(function () {
    ...
    datagrid = $("#dataGrid4Preview").datagrid({
        rownumbers: true, url: ctx + "/rms/rs/arrival-gate-list_4-mock", queryParams: { infoId: $("#infoId").val() }, method: 'get',
                        ...
                        if (selectedRow.flightDynamicNature !== '3') {
                            ...
                            layer.open({
                                ...
                                yes: function (index, layero) {
                                    ...
                                    commonManuallyDistributionFn(ctx + '/rms/rs/arrival_gate_manual_dist_4_mock', ctx + '/rms/rs/arrival_gate_manual_dist_4_mock', pArgs, gArgs, checkingIndex)
                                    return true;
                                }
                            });
        ...
        }], onDblClickRow: scroll2CallbackFn, rowStyler: commonRowStyler, onLoadSuccess: layoutAutomaticallyAdapter
    });
...
function clearCallbackFn () {
    ...
    if (selectedRow) {
        if (selectedRow.inteCarouselCode || selectedRow.intlCarouselCode) {
            layer.confirm(`是否要清除当前航班「${selectedRow.flightCompanyCode + selectedRow.flightDynamicCode}」分配的到港门？`, { icon: 3, title: '提示' }, function (index) {
...
function ganttReload () {
    $(".gantt").gantt('reload', ctx + '/rms/rs/disted_arrival_gate_Json_4_mock', { infoId: $("#infoId").val() })
}
    ...
    if (!_aircraft_num) {
        ...
        var confirmLayerIndex = layer.open({
            ...
            yes: function (index, layero) {
                ...
                commonManuallyDistributionFn(ctx + '/rms/rs/arrival_gate_manual_dist_4_mock', ctx + '/rms/rs/arrival_gate_manual_dist_4_mock', pArgs, gArgs, checkingIndex, confirmLayerIndex)
                return true;
            }
        });
    } else {
        ...
        if (draggable_holder) {  // 用户拖动Gantt上的元素
            ...
            if (!newAxisExists) {  // 移动到inte及intl都不存在的数值上
                ...
                layer.confirm(layerPromptMsgTpl, function (index) {
                    ...
                    commonManuallyDistributionFn(ctx + '/rms/rs/arrival_gate_manual_dist_4_mock', ctx + '/rms/rs/arrival_gate_manual_dist_4_mock', params, gArgs, checkingIndex)
                }, () => normalDragStopFn(origin_drag_elem));
