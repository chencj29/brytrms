...
var xAxis = enhanceXAxis4Mock(), yAxis = $utils.ajaxLoadFragments(ctx + '/rms/boardingGate/listJson'), checkingIndex;

var gateGanttOptions = {
    auto: true, xAxis: xAxis, yAxis: yAxis, draggableAxis: 'y',
    collisionThresholds: getThresholdValue(), timeline: true, resizableHandler: false, ajaxUrl: ctx + '/rms/rs/disted_boarding_gate_Json_4_mock',
    ajaxParams: { infoId: $("#infoId").val() }, asyncRenderCallback: asyncGanttElementRenderTaskFn,
    ...
    dblClick: function (event) {
        commonSetActualTimeHandler(TPL_NAME, {
            flightDynamicId: event.data.flightDynamicId, nature: event.data.extra.codeNature,
            inteStart: event.data.inteActualStartTime, inteOver: event.data.inteActualOverTime, title: event.data.text
        }, '登机口 - 实际占用时间设置', '/rms/rs/setBoardingGateActualTime');
    },
...
$(function () {
    ...
    datagrid = $("#dataGrid4Preview").datagrid({
        rownumbers: true, url: ctx + "/rms/rs/boarding-gate-list_4-mock", queryParams: { infoId: $("#infoId").val() }, method: 'get',
        idField: "id", singleSelect: true, showFooter: true, multiSort: false, fitColumns: true, fit: true, toolbar: [{
                ...
                if (selectedRow) {
                    if (multiModeGanttDistributedAvailableChecking(moduleName, '/rms/rs/getBoardingGateOccupyingTime?flightDynamicId=' + selectedRow.flightDynamicId, selectedRow)) {
                        if (selectedRow.flightDynamicNature !== '3') {
                            ...
                            layer.open({
                                title: '手动分配', btn: ['确定', '取消'], area: ['380px', '160px'], type: 1, scrollbar: false, move: false,
                                ...
                                yes: function (index, layero) {
                                    ...
                                    var pArgs = { flightDynamicId: selectedRow.detailId, inte: cintec, intl: cintlc, nature: !_.isEmpty(cintec) ? "1" : "2", force: false };
                                    var gArgs = {
                                    ...
                                    commonManuallyDistributionFn(ctx + '/rms/rs/boarding_gate_manual_dist_4_mock', ctx + '/rms/rs/boarding_gate_manual_dist_4_mock', pArgs, gArgs, checkingIndex)
                                    return true;
                                }
                            });
        ...
        }], onDblClickRow: scroll2CallbackFn, rowStyler: commonRowStyler, onLoadSuccess: layoutAutomaticallyAdapter, onClickCell: fetchVipInfos
    });
...
function clearCallbackFn () {
    ...
    if (selectedRow) {
        if (selectedRow.inteCarouselCode || selectedRow.intlCarouselCode) {
            layer.confirm(`是否要清除当前航班「${selectedRow.flightCompanyCode + selectedRow.flightDynamicCode}」分配的登机口？`, { icon: 3, title: '提示' }, function (index) {
...
function ganttReload () {
    $(".gantt").gantt('reload', ctx + '/rms/rs/disted_boarding_gate_Json_4_mock', { infoId: $("#infoId").val() })
}
...
function dist_boarding_gate_manually (_aircraft_num) {
    ...
    if (!_aircraft_num) {
        ...
        var confirmLayerIndex = layer.open({
            title: '登机口手动分配', btn: ['确定', '取消'], area: ['350px', selectedRow.flightDynamicNature === '3' ? '200px' : '180px'],
            type: 1, scrollbar: false, move: false,
            content: fetchResourceDistTpl(TPL_NAME, {
            ...
            yes: function (index, layero) {
                ...
                commonManuallyDistributionFn(ctx + '/rms/rs/boarding_gate_manual_dist_4_mock', ctx + '/rms/rs/boarding_gate_manual_dist_4_mock', pArgs, gArgs, checkingIndex, confirmLayerIndex)
                return true;
            }
        });
    } else {
        ...
        if (draggable_holder) {  // 用户拖动Gantt上的元素
            ...
            if (!newAxisExists) {  // 移动到inte及intl都不存在的数值上
                ...
                layer.confirm(layerPromptMsgTpl, function (index) {
                    ...
                    commonManuallyDistributionFn(ctx + '/rms/rs/boarding_gate_manual_dist_4_mock', ctx + '/rms/rs/boarding_gate_manual_dist_4_mock', params, gArgs, checkingIndex);
            ...
            if (intes === _aircraft_num || intls === _aircraft_num) { // 清除操作
                ...
                layer.confirm(tipMessage, function (index) {
                    ...
                    $.post(ctx + '/rms/rs/clearCarouselCode4Mock', params).done(function (data) {
                        layer.close(checkingIndex);
                        if (data.code == 1) {
                            layer.msg(data.message);
                            updateResourcePosition(selectedRow.id, { inteCarouselCode: params.inte, intlCarouselCode: params.intl });
                ...
                layer.confirm(tipMessage, function (index) {
                    ...
                    var gArgs = { selectedRowId: selectedRow.id, nInte: cintec, nIntl: cintlc, oInte: oldCarouselCode.inte, oIntl: oldCarouselCode.intl, force: true };
                    commonManuallyDistributionFn(ctx + '/rms/rs/boarding_gate_manual_dist_4_mock', ctx + '/rms/rs/boarding_gate_manual_dist_4_mock', pArgs, gArgs, checkingIndex)
                }, clearManuallyGanttRefs);
            }
        }
