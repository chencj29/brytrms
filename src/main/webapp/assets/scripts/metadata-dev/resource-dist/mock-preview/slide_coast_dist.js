...
var xAxis = enhanceXAxis4Mock(), yAxis = $utils.ajaxLoadFragments(ctx + '/rms/slideCoast/listJson');
...
var gateGanttOptions = {
    ...
    ajaxUrl: ctx + '/rms/rs/disted_slide_coast_Json_4_mock', ajaxParams: { infoId: $("#infoId").val() },
    asyncRenderCallback: asyncGanttElementRenderTaskFn, move2HoldAreaCallback: function (oData) {
        if (oData.flightDynamicId) { // 用户手动拖动到「预留区」
            ...
            var originAxis = origin_drag_target[0], selectedRow = enhanceGetDGSelectedRow(oData.id),
                tipMessage = `您确定要清除「${selectedRow.flightCompanyCode + selectedRow.flightDynamicCode}」航班的「${originAxis.text}」号滑槽吗?`;
    ...
    dblClick: function (event) {
        ...
        }, '滑槽 - 实际占用时间设置', '/rms/rs/setSlideCoastActualTime');
    },
...
$(function () {
    ...
    datagrid = $("#dataGrid4Preview").datagrid({
        rownumbers: true, url: ctx + "/rms/rs/slide-coast-list_4-mock", queryParams: { infoId: $("#infoId").val() }, method: 'get',
                ...
                if (selectedRow) {
                    if (multiModeGanttDistributedAvailableChecking(moduleName, '/rms/rs/getSlideCoastOccupyingTime?flightDynamicId=' + selectedRow.flightDynamicId, selectedRow)) {
                        if (selectedRow.flightDynamicNature !== '3') {
                            ...
                            layer.open({
                                title: '手动分配', btn: ['确定', '取消'], area: ['370px', '150px'], type: 1, scrollbar: false, move: false,
                                ...
                                yes: function (index, layero) {
                                    ...
                                    commonManuallyDistributionFn(ctx + '/rms/rs/slide_coast_manual_dist_4_mock', ctx + '/rms/rs/slide_coast_manual_dist_4_mock', pArgs, gArgs, checkingIndex)
                                    return true;
                                }
                            });
...
function clearCallbackFn () {
    ...
    if (selectedRow) {
        if (selectedRow.inteCarouselCode || selectedRow.intlCarouselCode) {
            layer.confirm(`是否要清除当前航班「${selectedRow.flightCompanyCode + selectedRow.flightDynamicCode}」分配的滑槽？`, { icon: 3, title: '提示' }, function (index) {
...
function ganttReload () {
    $(".gantt").gantt('reload', ctx + '/rms/rs/disted_slide_coast_Json_4_mock', { infoId: $("#infoId").val() });
}
...
function dist_slide_coast_manually (_aircraft_num) {
    ...
    if (!_aircraft_num) {
        ...
        var confirmLayerIndex = layer.open({
            title: '滑槽手动分配', btn: ['确定', '取消'], area: ['370px', selectedRow.flightDynamicNature === '3' ? '199px' : '150px'],
            ...
            yes: function (index, layero) {
                ...
                var pArgs = { flightDynamicId: selectedRow.detailId, inteSlideCoastCode: cintec, intlSlideCoastCode: cintlc, nature: selectedRow.flightDynamicNature, force: false };
                ...
                commonManuallyDistributionFn(ctx + '/rms/rs/slide_coast_manual_dist_4_mock', ctx + '/rms/rs/slide_coast_manual_dist_4_mock', pArgs, gArgs, checkingIndex, confirmLayerIndex)
                return true;
            }
        });
    } else {
        ...
        if (draggable_holder) {  // 用户拖动Gantt上的元素
            ...
            if (!newAxisExists) {  // 移动到inte及intl都不存在的数值上
                ...
                layer.confirm(layerPromptMsgTpl, function (index) {
                    ...
                    commonManuallyDistributionFn(ctx + '/rms/rs/slide_coast_manual_dist_4_mock', ctx + '/rms/rs/slide_coast_manual_dist_4_mock', params, gArgs, checkingIndex);
            ...
            if (intes === _aircraft_num || intls === _aircraft_num) { // 清除操作
                ...
                layer.confirm(tipMessage, function (index) {
                    ...
                    var pArgs = { flightDynamicId: selectedRow.detailId, nature: nature, inte: cintec, intl: cintlc, force: false }
