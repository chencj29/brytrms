...
$(function () {
    ...
    if (!flagAutoHight) {
        ...
        c.layout('panel', 'south').panel('resize', {height: h>200?200:h});
    ...
    toolbarDate.push({
        ...
        iconCls: "icon-undo",
        handler: function () {
            $('#scheduling_history_dialog').dialog('open');
        }
    });
    ...
    $("#btnSave").click(function () {
        if ($(this).linkbutton('options').disabled == false && $('#modifyForm').form('validate')) {
            ...
            data.workingType = $('#working_type_form').combobox('getText');
            data.workingArea = $('#working_area_form').combobox('getText');
            $.post(ctx + "/rms/" + moduleContext + "/save", data, function (data) {
                if (data.code === 1) datagrid.datagrid("reload");
                $.messager.show({title: "提示", msg: data.message, timeout: 5000, showType: 'slide'});
            });
        }else{
            $.messager.slide('验证未通过!');
        }
    });
