		...
		top.$.jBox.open("iframe:${ctx}/tag/treeselect?url="+encodeURIComponent("${url}")+"&module=${module}&checked=${checked}&extId=${extId}&isAll=${isAll}", "选择${title}", 300, 420, {
			ajaxData:{selectIds: $("#${id}Id").val()},buttons:{"确定":"ok", ${allowClear?"\"清除\":\"clear\", ":""}"关闭":true}, submit:function(v, h, f){
				if (v=="ok"){
					...
					var k={};$.each(${fns:toJson(fns:getDictList('sys_office_type'))},function(i,v){k[v.value]=v.label});//类型-->值 eg:{4:"岗位"}
					...
					//wjp_2016年8月25日15时29分 识别岗位节点
					if( nodes.length==1 && type==5 && k[nodes[0].type]!="岗位"){  //将url中type = 5设为岗位选择判断节点 wjp_2016年8月9日11时40分
						//$("#"+treeId).jbox("请选择岗位!");
						top.$.jBox.tip("你选择的是:【"+k[nodes[0].type]+"】节点；<br\>请选择【岗位】节点！");
						return false;
					}
