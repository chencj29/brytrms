...
<script>
    var moduleContext = "rmsHiFlightHomeManifest", moduleContext_old = "rmsFlightHomeManifest";
    ...
    <shiro:hasPermission name="rms:rmsFlightHomeManifest:hiExpExcel">flagDgExp = true;
    </shiro:hasPermission>
    ...
    function getManifestSecListAndfilld(flightDynamicId) {
        if (flightDynamicId) {
            $.ajax({
                type: "post",
                async: false,
                url: "${ctx}/rms/" + moduleContext_old + "/getHiManifestSecs",
    ...
    $(function () {
        ...
        $("#btnSave").click(function () {
            if ($(this).linkbutton('options').disabled == false && $('#modifyForm').form('validate')) {
                ...
                $.post(ctx + "/rms/" + moduleContext_old + "/hiSaveSec", $("#sce_modifyForm").serialize(), function (data) {
                    $.messager.show({title: "提示", msg: data.message, timeout: 5000, showType: 'slide'});
                });
        ...
        /*if (!flagAutoHight) {
         var c = $('#datagrid').parents('.easyui-layout').eq(0);
         c.layout('panel', 'south').panel('resize', {height: 300});
         }*/
