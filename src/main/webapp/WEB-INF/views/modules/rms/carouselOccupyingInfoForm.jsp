...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/rms/carouselOccupyingInfo/">行李转盘占用信息列表</a></li>
		<li class="active"><a href="${ctx}/rms/carouselOccupyingInfo/form?id=${carouselOccupyingInfo.id}">行李转盘占用信息<shiro:hasPermission name="rms:carouselOccupyingInfo:edit">${not empty carouselOccupyingInfo.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="rms:carouselOccupyingInfo:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="carouselOccupyingInfo" action="${ctx}/rms/carouselOccupyingInfo/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			<label class="control-label">预计占用行李转盘个数：</label>
			<div class="controls">
				<form:input path="expectedCarouselNum" htmlEscape="false" maxlength="18" class="input-xlarge  digits"/>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">实际占用行李转盘个数：</label>
			<div class="controls">
				<form:input path="actualCarouselNum" htmlEscape="false" maxlength="18" class="input-xlarge  digits"/>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">行李转盘编号(国内)：</label>
			<div class="controls">
				<form:input path="inteCarouselCode" htmlEscape="false" maxlength="100" class="input-xlarge "/>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">行李转盘编号(国际)：</label>
			<div class="controls">
				<form:input path="intlCarouselCode" htmlEscape="false" maxlength="100" class="input-xlarge "/>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="form-actions">
			<shiro:hasPermission name="rms:carouselOccupyingInfo:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
